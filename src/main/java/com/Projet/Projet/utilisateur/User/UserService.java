...
public class UserService implements ServiceUtilisateur {
    ...
    public ResponseEntity<?> authenticateUser(LoginRequest loginRequest) {
        try {
            Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(loginRequest.getTelephoneOrEmail(), loginRequest.getPassword()));
...
//            List<String> specialites = userDetails.getSpecialites();
    ...
    public ResponseEntity<?> registerUser(SignupRequest signUpRequest) {
       ...
       if (strRoles == null) {
           Role resPonRole = roleRepository.findByName(ERole.ROLE_CLIENT)
           ...
           strRoles.forEach(role -> {
               switch (role) {
                   case "admin":
                       ...
                       break;
                   case "client":
                       Role mecanoRole = roleRepository.findByName(ERole.ROLE_CLIENT)
                               .orElseThrow(() -> new RuntimeException("Error: Le rôle client n'a pas été trouvé."));
                       roles.add(mecanoRole);
                       break;
//                   case "resPonRole":
//                       Role resPonRole = roleRepository.findByName(ERole.ROLE_CLIENT)
//                               .orElseThrow(() -> new RuntimeException("Error: Le rôle responsable maintenance n'a pas été trouvé."));
//                       roles.add(resPonRole);
//                       break;
                   case "informaticien":
                       Role conducteurRole = roleRepository.findByName(ERole.ROLE_INFORMATICIEN)
                               .orElseThrow(() -> new RuntimeException("Error: Le rôle informaticien n'a pas été trouvé."));
    ...
    public MessageResponse Bannir(User user) {
        return userRepository.findById(user.getId())
                .map(p -> {
                    ...
                    String message = "L'utilisateur a été mis à jour avec succès";
